  ...
  try {
    ...
    if (current?.fetchedAt) {
      ...
      if (age < COOLDOWN_MS) {
        ...
        // No new LegiScan pull, but this still counts as someone having
        // checked — record it separately from fetchedAt so the UI can show
        // "last checked" even when there's nothing new to report.
        const lastCheckedAt = new Date().toISOString();
        await store.setJSON("latest.json", { ...current, lastCheckedAt });
        return new Response(
          JSON.stringify({
            ok: true,
            skipped: true,
