...
export async function fetchAndStoreCompactData(): Promise<{
  ...
  // lastCheckedAt starts equal to fetchedAt — any real LegiScan pull (cron
  // or a manual check past cooldown) counts as both an update AND a check.
  // A manual check that gets skipped by the cooldown (see public-refresh.mts)
  // bumps lastCheckedAt alone, without touching fetchedAt.
